...
function generatePDF() {
  ...
  function addHeader(title, withLogo = false) {
    ...
    doc.rect(0, bannerH, W, 2, 'F');
    
    // Logo links bovenin
    if (withLogo) {
      try { doc.addImage(LOGO_123BE, 'JPEG', M, 8, 45, 15); } catch(e) {}
    }
    ...
    doc.text(title, withLogo ? M + 52 : M, 28);
    
    // Subtitle line
    if (!withLogo) {
      doc.setFontSize(9);
      doc.setTextColor(255, 255, 255);
      doc.text('Ground Research BV  |  123Bodemenergie', M, 36);
    }
